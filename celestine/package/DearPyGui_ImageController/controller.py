...
ImageControllerType = TypeVar(
    "ImageControllerType", bound="ImageController"
)
...
class ImageController:
    ...
    def subscribe(
        self, image_viewer: Type[ImageViewerCreator]
    ) -> SubscriptionTag:
    ...
    def is_unloading_time(self) -> bool:
        if self.image:
            return (
                time.time() - self.last_time_visible
            ) > self.controller.max_inactive_time
        return True
...
class ImageLoaderWorker(Worker):
    ...
    def load(image_controller: ImageController):
        if not image_controller.loading:
            return
        if (
            image_controller.is_unloading_time()
            or image_controller.loaded
        ):
...
class Controller(Dict[ImageControllerTag, ImageController]):
    ...
    def __init__(
        ...
        self.unload_queue = []
        self.unloading_worker = ImageUnloaderWorker(
    ...
    def add(
        ...
        image_info = ImageController(
            image=image, tag_in_controller=image_tag, controller=self
        )
    ...
    def unload_images(self, max_count: int = None):
        ...
        if not self.disable_work_in_threads:
            return
        if (
            time.time() - self._last_time_unload_check
            < self.unloading_check_sleep_time
        ):
            return

...
class ImageViewerCreator(ABC):
    ...
    def load(
        ...
        self.__subscription_tag = self.__image_info.subscribe(
            self
        )  # noqa
...
class ImageViewer(ImageViewerCreator):
        ...
        if not self._visible_handler:
            with dpg.item_handler_registry() as self._visible_handler:
                dpg.add_item_visible_handler(
                    callback=self.update_last_time_visible,
                    parent=self._visible_handler,
                )
    ...
    def get_size(self) -> (int, int):
        ...
        if self.width:
            height = int(
                self.image.height * (self.width / self.image.width)
            )
            return self.width, height
        if self.height:
            width = int(
                self.image.width * (self.height / self.image.height)
            )
    ...
    def set_size(self, *, width: int = None, height: int = None):
        ...
        try:
            dpg.configure_item(
                self._view_window, width=width, height=height
            )
            if self.dpg_image:
                dpg.configure_item(
                    self.dpg_image, width=width, height=height
                )
        except Exception:
            traceback.print_exc()
        ...
        if self.dpg_image:
            try:
                dpg.bind_item_handler_registry(
                    self.dpg_image, self.image_handler
                )
            except Exception:
                pass
    ...
    def create(
        ...
        with dpg.group(parent=parent) as self.group:
            ...
            self._view_window = dpg.add_child_window(
                width=width,
                height=height,
                no_scrollbar=True,
                parent=self.group,
            ...
            dpg.bind_item_handler_registry(
                self.group, self._get_visible_handler()
            )
